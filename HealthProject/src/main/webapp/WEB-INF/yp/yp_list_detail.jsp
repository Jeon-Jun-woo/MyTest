...
<sec:authorize access="isAuthenticated()">
	...
	<div class="container" id="ypApp">
		<div class="banner-section spad">
	        <div class="container-fluid">
	        	<h2 class="sectiontitle">요가/필라테스 상세보기</h2>
	        	<table class="table">
	            	<tr>
	              		<td colspan="2" class="text-center">
	                		<img :src="yp_detail.poster" style="height: 300px">
	              		</td>
	            	</tr>
	            	<tr>
	              		<td colspan="2">
	                		<h3><span id="name">{{yp_detail.title}}</span>&nbsp;<span style="color:orange"><%-- {{food_detail.score}} --%></span></h3>
	              		</td>
	            	</tr>
	            	<tr>
	              		<th width="15%">주소</th>
	            	...
	            	<tr>
	                	...
	                	<!-- 	<a href="../reserve/reserve_main.do"><input type="button" class="btn-sm btn-info" value="예약하기"></a>&nbsp; -->
	                		<input type="button" class="btn-sm btn-warning" value="목록으로 돌아가기" @click="goback()">
	              		</td>
	            	</tr>
	            </table>
			</div>
    ...
    <script>
	  let ypApp=Vue.createApp({
		  ...
		  mounted(){
			  axios.get('../yp/yp_detail_vue.do',{
				  params:{
					  no:this.no
				  }
			  }).then(response=>{
				  console.log(response.data)
				  this.yp_detail=response.data.detail_data // .detail_data를 추가함
		  ...
		  methods:{
			  replyUpdate(no){
		    	  ...
		    	  //alert(msg)
		    	  axios.post('../yp/reply_update_vue.do', null, {
		    		  params:{
		    			  no:no,
		    			  yno:this.no,
		    			  msg:msg
		      ...
		      replyDelete(no){
		    	  axios.get('../yp/reply_delete_vue.do',{
		    		  params:{
		    			  no:no,
		    			  yno:this.no
		    		  }
		      ...
		      replyInsert(){
		    	  ...
		    	  axios.post('../yp/reply_insert_vue.do',null,{
		    		  params:{
		    			  yno:this.no,
		    			  msg:this.msg
		    		  }
			  ...
			  initMap(){
	    		...
	    		// 주소로 좌표를 검색합니다
	    		geocoder.addressSearch(this.yp_detail.address, function(result, status) {
			  ...
			  goback(){
				  ...
				  location.href="../yp/yp_list.do" // 이렇게 해야 함
			  }
		  }
	    }).mount("#ypApp")
	  </script>
